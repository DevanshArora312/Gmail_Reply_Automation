...
async function hasRepliedToMessage(auth, messageId) {
    ...
    try {
        ...
        const hasSentByYou = headers.some(header => header.name === 'From' && header.value.includes('devansharora312@gmail.com'));
        // console.log(response.data.payload);
        return {hasReplied : hasSentByYou,headers : headers};
    } catch (error) {
        console.error('Error checking if the message has been replied to:', error.message);
        throw error;
    }
}
